...
## Recent changes
- 2026-06-10 — Automated content pipeline: `.github/workflows/auto-content.yml` runs Mon/Thu (and on manual dispatch), has Claude write the first unchecked item from `docs/content-backlog.md` (the editorial queue — learn articles, guides, or clinic reviews), verify it renders, and open a PR for human approval. Requires `ANTHROPIC_API_KEY` repo secret; cron only fires on the default branch.
